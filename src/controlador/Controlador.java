...
public class Controlador implements ActionListener
{
	...
	/**
	 * metodo que permite llamar al metodo de la interfaz IVista para la accion
	 * correspondiente segun el tipo de evento recibido de la interfaz de
	 * usuario<br>
	...
	/**
	 * metodo que acciona la invocacion del metodo agrega pacientes y medicos,
	 * invoca a otro metodo para actualizar las listas y tambien se encarga de
	...
	public void Agregar(String comando) {
		if (this.vista.getTipoAgregarSeleccionado() == "Paciente") // recibe action comand
		{
			try {
				...
				IPaciente paciente = PacienteFactory.getPaciente(this.vista.getDNINuevo(), this.vista.getNomNuevo(),
						this.vista.getTelNuevo(), this.vista.getDomNuevo(), this.vista.getCiudadNuevo(),
						this.vista.getMatriculaNuevo(), this.vista.getRangoEtario());
		...
		if (this.vista.getTipoAgregarSeleccionado() == "Medico") {
			try {
				IMedico medico = MedicoFactory.getMedico(this.vista.getNomNuevo(), this.vista.getDNINuevo(),
						this.vista.getDomNuevo(), this.vista.getCiudadNuevo(), this.vista.getTelNuevo(),
						this.vista.getMatriculaNuevo(), this.vista.getEspecialidad(), this.vista.getContratacion(),
						this.vista.getPosgrado(), this.vista.getHonorario());
	...
	/**
	 * metodo que acciona el metodo que elimina medicos y pacientes de las listas
	 * del sistema de la Interfaz IVista, se invoca al metodo actualiza listas y se
	 * valida los datos <br>
	 */
	...
	/**
	 * metodo que acciona la realizacion de una prestacion en el modelo y valida los
	 * datos antes
	 */
		...
		if (paciente != null) // hay alguien
		{
			...
			if (medico != null) // tiene medico
			{
				...
				if (habitacion != null) {
					...
					this.optionpane.ShowMessage(null, Mensajes.Seleccione_Habitacion.getValor());
			} else
				this.optionpane.ShowMessage(null, Mensajes.Seleccione_Medico.getValor());
		} else
	...
	public void Facturar() {
		...
		if (pac != null) {
			try {
				String factura = Clinica.getInstance().egreso(pac, new GregorianCalendar()); // cambiar el gregorian
																								// calendar
	...
	/**
	 * metodo que acciona al metodo llamar paciente del modelo, valida que haya
	 * pacientes en espera
	 */
	...
	/**
	 * metodo que acciona al metodo ingreso de la clinica, valida que el paciente no
	 * este ingresado
	 */
	public void Ingreso() {
	...
	/**
	 * metodo que guarda el estado actual del sistema luego de que alguna accion se
	 * produjo
	 */
